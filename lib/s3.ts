...
class EasyYandexS3 {
  ...
  async _uploadDirectory(dir, params, route, ignore_list) {
    ...
    for (let thread_files of threads) {
      for (let file of thread_files) {
        ...
        if (debug)
          this._log('S3', debug_object, 'dir file uploading:', route, 'to', file.relative_dir_path);
  ...
  async GetList(route, ContinuationToken) {
    if (!route) route = '/';
    if (route == './') route = '/';
...
function readDir(dir_path, original_file_path, ignore_list) {
  ...
  for (var file_name of dir_files) {
    ...
    const relative_dir_path = relative_file_path.replace(file_name, '');

    if (ignore_list.includes(file_name)) continue;
    if (
      ignore_list.includes(relative_file_path.replace(/\\/g, '/')) ||
      ignore_list.includes(relative_file_path.replace(/\//g, '\\'))
    )
      continue;
