...
export interface ToggleAttributeInRubricFilterInputInterface {
  rubricId: string;
  attributeIds: string[];
  attributesGroupId: string;
}

export async function toggleAttributeInRubricFilter({
  context,
  input,
  ...
  try {
    ...
    console.log('toggleAttributeInRubricFilter error', e);
    return {
      success: false,
