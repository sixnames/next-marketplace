export const COL_ATTRIBUTES = 'attributes';
export const COL_ATTRIBUTES_GROUPS = 'attributesGroups';
export const COL_BLACKLIST_PRODUCTS = 'blackListProducts';
export const COL_BLOG_ATTRIBUTES = 'blogAttributes';
export const COL_BLOG_LIKES = 'blogLikes';
export const COL_BLOG_POSTS = 'blogPosts';
export const COL_BRANDS = 'brands';
export const COL_BRAND_COLLECTIONS = 'brandCollections';
export const COL_CARTS = 'carts';
export const COL_CATALOGUE_NAV = 'catalogueNav';
export const COL_CATEGORIES = 'categories';
export const COL_CITIES = 'cities';
export const COL_COMPANIES = 'companies';
export const COL_CONFIGS = 'configs';
export const COL_COUNTRIES = 'countries';
export const COL_CURRENCIES = 'currencies';
export const COL_EVENT_FACETS = 'eventFacets';
export const COL_EVENT_RUBRICS = 'eventRubrics';
export const COL_EVENT_SUMMARIES = 'eventSummaries';
export const COL_GIFT_CERTIFICATES = 'giftCertificates';
export const COL_ICONS = 'icons';
export const COL_ID_COUNTERS = 'idCounters';
export const COL_LANGUAGES = 'languages';
export const COL_MANUFACTURERS = 'manufacturers';
export const COL_MESSAGES = 'messages';
export const COL_MESSAGES_GROUPS = 'messagesGroups';
export const COL_METRICS = 'metrics';
export const COL_NAV_ITEMS = 'navItems';
export const COL_NOT_SYNCED_PRODUCTS = 'notSyncedProducts';
export const COL_OPTIONS = 'options';
export const COL_OPTIONS_GROUPS = 'optionsGroups';
export const COL_ORDERS = 'orders';
export const COL_ORDER_CUSTOMERS = 'ordersCustomers';
export const COL_ORDER_LOGS = 'ordersLogs';
export const COL_ORDER_PRODUCTS = 'ordersProducts';
export const COL_ORDER_STATUSES = 'orderStatuses';
export const COL_PAGES = 'pages';
export const COL_PAGES_GROUP = 'pagesGroups';
export const COL_PAGES_GROUP_TEMPLATES = 'pagesGroupTemplates';
export const COL_PAGE_TEMPLATES = 'pageTemplates';
export const COL_PRODUCT_FACETS = 'productFacets';
export const COL_PRODUCT_SUMMARIES = 'productSummaries';
export const COL_PROMO = 'promo';
export const COL_PROMO_CODES = 'promoCodes';
export const COL_PROMO_PRODUCTS = 'promoProducts';
export const COL_ROLES = 'roles';
export const COL_ROLE_RULES = 'roleRules';
export const COL_RUBRICS = 'rubrics';
export const COL_RUBRIC_VARIANTS = 'rubricVariants';
export const COL_SEO_CONTENTS = 'seoContents';
export const COL_SESSION_LOGS = 'sessionLogs';
export const COL_SHOPS = 'shops';
export const COL_SHOP_PRODUCTS = 'shpProducts';
export const COL_SUPPLIERS = 'suppliers';
export const COL_SUPPLIER_PRODUCTS = 'supplierProducts';
export const COL_SYNC_INTERSECT = 'syncIntersect';
export const COL_SYNC_LOGS = 'syncLogs';
export const COL_TASKS = 'tasks';
export const COL_TASK_VARIANTS = 'taskVariants';
export const COL_USERS = 'users';
export const COL_USER_CASHBACK_LOGS = 'userCashbackLogs';
export const COL_USER_CATEGORIES = 'userCategories';
export const COL_USER_PAYBACK_LOGS = 'userPaybackLogs';
